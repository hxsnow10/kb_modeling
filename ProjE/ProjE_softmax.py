...
class ProjE:
    ...
    def __init__(self, data_dir, embed_dim=100, combination_method='simple', dropout=0.5, neg_weight=0.5):
        ...
        with open(os.path.join(data_dir, 'entity2id.txt'), 'r', ) as f:
            self.__entity_id_map = {x.strip().split()[0]: k for k,x in enumerate(f.readlines())}
        ...
        with open(os.path.join(data_dir, 'relation2id.txt'), 'r', ) as f:
            self.__relation_id_map = {x.strip().split()[0]: k for k,x in enumerate(f.readlines())}
            print(self.__relation_id_map.keys())
        ...
        self.__train_triple = load_triple(os.path.join(data_dir, 'train2id.txt'))
        print("N_TRAIN_TRIPLES: %d" % self.__train_triple.shape[0])

        self.__test_triple = load_triple(os.path.join(data_dir, 'test2id.txt'))
        print("N_TEST_TRIPLES: %d" % self.__test_triple.shape[0])

        self.__valid_triple = load_triple(os.path.join(data_dir, 'valid2id.txt'))
